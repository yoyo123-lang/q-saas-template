...
import { notifyUserActivated } from "@/lib/sync/user-sync";
...
/** Notifica a Q-Company que el usuario completó onboarding en esta BU (fire-and-forget). */
function fireNotifyActivated(userId: string): void {
  prisma.user
    .findUnique({ where: { id: userId }, select: { email: true } })
    .then((u) => {
      if (u?.email) notifyUserActivated(u.email).catch(() => {});
    })
    .catch(() => {});
}
...
const STEP_ROUTES: Record<string, string> = {
  ...
  // BU-SPECIFIC STEPS: agregar rutas acá al personalizar
  // Ejemplo: "first-product": "/onboarding/first-product",
};
...
export async function skipOnboardingStep(): Promise<void> {
  ...
  if (!currentStepConfig || currentStepConfig.required) {
    // No debería llegar aquí, pero redirigir al paso actual por seguridad
...
// ─── BU-SPECIFIC STEPS ───────────────────────────────────────────────────────
//
// Agregar acá las server actions específicas de tu BU.
// Cada acción debe:
//   1. Validar input con Zod
//   2. Obtener el progress activo del usuario
//   3. Realizar la operación de negocio en una transacción
//   4. Actualizar onboardingProgress.currentStep
//   5. Redirigir al siguiente paso o al dashboard si era el último
//
// Ejemplo de estructura:
//   export async function saveFirstProductStep(input: FirstProductInput) { ... }
//   export async function saveFirstContactStep(input: FirstContactInput) { ... }
